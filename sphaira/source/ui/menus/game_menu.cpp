...
void Menu::ScanHomebrew() {
    ...
    while (true) {
        ...
        for (s32 i = 0; i < record_count; i++) {
            ...
            const auto app_id = GetBaseApplicationTitleId(e.application_id);
            if (hide_forwarders && (app_id & 0x0500000000000000) == 0x0500000000000000) {
                continue;
            }

            if (std::ranges::find(seen_app_ids, app_id) != seen_app_ids.end()) {
                continue;
            }

            seen_app_ids.emplace_back(app_id);
            m_entries.emplace_back(app_id, e.last_event);
        }
